...
var closableTab = {
	...
	addTab:function(tabItem){ //tabItem = {id,name,url,closable}
		...
		if(!$('#'+id)[0]){
			...
			if(tabItem.closable){
				li_tab = li_tab + '<span class="glyphicon glyphicon-remove small" tabclose="'+id+'" style="position: absolute;right:5px;top: 7px;font-size: 10px;cursor:pointer;"  onclick="closableTab.closeTab(this)"></span></a></li> ';
			}else{
				li_tab = li_tab + '</a></li>';
			}
